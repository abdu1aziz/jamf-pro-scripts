...
##############################################################################
# Install Packages below, as provisioning process for the macOS setup.
##############################################################################

/usr/bin/su - "$current_user" -c brew install --cask google-chrome
/usr/bin/su - "$current_user" -c brew install --cask visual-studio-code
/usr/bin/su - "$current_user" -c brew install --cask openvpn-connect
/usr/bin/su - "$current_user" -c brew install --cask telegram-desktop
/usr/bin/su - "$current_user" -c brew install --cask signal
/usr/bin/su - "$current_user" -c brew install --cask spotify
/usr/bin/su - "$current_user" -c brew install --cask whatsapp
/usr/bin/su - "$current_user" -c brew install --cask zoom
/usr/bin/su - "$current_user" -c brew install --cask megasync
/usr/bin/su - "$current_user" -c brew install --cask discord
/usr/bin/su - "$current_user" -c sh -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"

########################################
# Change Hostname For macOS Device...
########################################
